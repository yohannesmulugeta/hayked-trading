...
All current photography is locally stored as optimized WebP files under `public/uploads`. The homepage hero sequence is stored under `public/uploads/hero` and is configured through `heroSlides` in `src/content/pages/home.json`. These are illustrative free-use images, not claimed as Hayked facilities, staff or owned photography. See `public/uploads/IMAGE_CREDITS.md`.
